...
## Acceptance Criteria
- [x] `along hook eval` executes and evaluates event payloads passed via stdin or args.
- [x] All 13 topic docs are discoverable in `mkdocs.yml` navigation.
- [x] `docs/topic--cli-reference.md` has a single valid YAML frontmatter block.
- [x] Skill count consistently states 19 across all docs and CLI docstrings.
- [x] `python scripts/along_kb_sync.py --check` passes without broken links.
